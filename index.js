...
//   );
//
// These connect over remote Streamable HTTP (not stdio), since that's what
// a URL-based "add a connector" UI can realistically configure — the
// stdio-based servers above (Postgres, Brave) stay hardcoded/env-based.
...
async function connectDynamicConnector(row) {
  if (!_McpHttpTransport) return;
  try {
    const headers = {};
    if (row.auth_header) headers["Authorization"] = row.auth_header;
    const transport = new _McpHttpTransport(new URL(row.url), {
      requestInit: { headers },
    });
    ...
    console.log(`🔌 MCP connector connected: ${row.label} (${tools.length} tools: ${tools.map((t) => t.name).join(", ")})`);
  } catch (e) {
...
async function autonomousTick() {
  ...
  try {
    ...
    const localHour = new Date(nowInTimezone().iso).getHours();
