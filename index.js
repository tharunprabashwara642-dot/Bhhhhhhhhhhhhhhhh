...
function describeAction(name, args) {
  ...
  switch (name) {
    ...
    case "deploy_website": return `🌐 Website එකක් හදලා deploy කරන්නද — "${args.description}"? (code ලියලා Vercel එකට යවනවා, පොඩි වෙලාවක් යනවා)`;
...
bot.on("callback_query", async (query) => {
  ...
  try {
    ...
    if (action === "confirm") {
      ...
      const result = await runToolDirectly(pc.toolName, pc.args);
