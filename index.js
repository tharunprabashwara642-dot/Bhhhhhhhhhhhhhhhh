...
async function getGoogleAccessToken() {
  ...
  if (!data.access_token) throw new Error(data.error_description || data.error || "Failed to refresh Google token");
...
async function fetchGeminiRotating(urlBuilder, options) {
  ...
  for (let i = 0; i < attempts; i++) {
    ...
    try {
      ...
      const data = await res.json();
      if (res.status === 429) {
        console.error(`⚠️ Key ...${key.slice(-4)} rate-limited (429), rotating to next key`);
...
async function checkScheduledResearch() {
  ...
  for (const task of dueTasks) {
    ...
    try {
      const result = await runResearch(task.topic);
      ...
      await bot.sendMessage(CHAT_ID, `⚠️ Research on "${task.topic}" failed: ${e.message}`);
    }
  }
}
...
async function scheduleResearch(topic, runAt, recurrence) {
  const { error } = await supabase
    .from("scheduled_tasks")
    .insert({ topic, run_at: runAt, status: "pending", recurrence: recurrence || "once" });
...
// ============================================================
// PART 4 — passive memory extraction
// Every 10 min: read unprocessed messages, ask Gemini what's worth
...
// ============================================================
// PART 5 — autonomous thinking loop
// Every 20 min: look at memories + active goals + recent chat + the
...
background process, running silently every ~20 minutes even when the user
...
- The user recently mentioned genuinely liking or being curious about
  something new (a topic, hobby, show, idea) in conversation — like a good
  friend, look into it and share something back. Use schedule_research with
  run_at set to right now (call get_current_datetime first) so it lands
  within a minute. Phrase the research topic itself as "something
  interesting/useful about X for a friend who's curious about it" so the
  result reads like a friend passing something along, not a formal report.
  Don't do this for every passing mention — only real interest, and never
  research the same topic twice (check recent conversation and memories).
...
nudge about the same thing you already sent recently (check the recent
conversation before acting). When you are completely done acting for this
tick (including if you decided to do nothing), your FINAL reply must be
exactly: NOTHING — unless you want to send the user a message right now, in
which case your final reply is that short warm message instead (2-3
sentences, no markdown).`;

setInterval(autonomousTick, 20 * 60 * 1000);
