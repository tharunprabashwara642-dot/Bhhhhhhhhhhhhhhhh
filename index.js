...
function convertJsonSchemaToGemini(schema) {
  ...
  if (out.type === "OBJECT") {
    ...
    for (const [key, val] of Object.entries(schema.properties || {})) {
      out.properties[key] = convertJsonSchemaToGemini(val);
    }
    if (schema.required && schema.required.length) out.required = schema.required;
  }
  if (out.type === "ARRAY") {
    out.items = convertJsonSchemaToGemini(schema.items || { type: "string" });
  }
  return out;
}
...
function registerCustomToolInMemory(name, description, parameters, code) {
  ...
  const decl = {
    ...
    parameters: convertJsonSchemaToGemini(params),
  };
...
async function editOwnCode(oldStr, newStr, commitMessage) {
  ...
  if (oldStr === newStr) {
    return { error: true, message: "old_str and new_str are identical — nothing to change." };
  }
  ...
  if (firstIdx === -1) {
    return { error: true, message: "old_str was not found in the current source — it must match exactly, whitespace included. Call read_own_code (or re-check the relevant section) and copy the text precisely, then retry." };
  }
...
const CREDENTIAL_PATTERNS = [
  ...
  { kind: "api_token", re: /\b(?:sk-[A-Za-z0-9_\-]{16,}|ghp_[A-Za-z0-9]{20,}|github_pat_[A-Za-z0-9_]{20,}|AIza[0-9A-Za-z_\-]{20,}|xox[baprs]-[A-Za-z0-9\-]{10,}|[A-Za-z0-9_\-]{32,})\b/ },
];
    ...
    try {
      if (item.kind === "gemini_key") {
        ...
        results.push(r.added
          ? `🔑 Gemini key added to the rotation — now cycling across ${r.total_keys} key(s)${r.persisted ? "" : " (⚠️ couldn't persist for restart — re-paste after redeploy)"}.`
          : `ℹ️ ${r.reason}`);
...
- read_own_code / edit_own_code / update_own_code (button-confirmed): fix
  a bug in yourself or change a hardcoded capability. Prefer edit_own_code
  — give it the exact old_str snippet (copy it precisely, whitespace
  included, from a read_own_code call) and the new_str to replace it with;
  it round-trips only that snippet, not the whole file, so it can't be
  truncated by output limits the way a full rewrite can. old_str must be
  unique in the file — include enough surrounding lines to pin down one
  spot. Only fall back to update_own_code (send back the COMPLETE file)
  for changes too large or structural for one or two snippet edits. If
  OWN_CODE_REPO is configured it commits to GitHub and Railway redeploys
  you with the new code; otherwise it writes the local file for the next
  restart.
...
const CHAT_TOOLS = [
  {
    functionDeclarations: [
      ...
      {
        name: "edit_own_code",
        description: "PREFERRED way to change your own source. Find-and-replace a single exact snippet of your own source file (button-confirmed, syntax-checked before writing) — like a str_replace edit. old_str must match the current file's text exactly (whitespace included) and must be unique in the file; include enough surrounding lines to pin down one location. Only the two snippets you send round-trip through the model — the file itself is never re-sent in full. Use this instead of update_own_code for any targeted fix or small addition.",
        parameters: {
          type: "OBJECT",
          properties: {
...
This step (step ${step.step_number}): "${step.description}"`;
  ...
  for (let i = 0; i < MAX_STEP_ROUNDS; i++) {
    let data;
    try {
      data = await callGemini(contents, stepInstruction);
    } catch (e) {
...
async function callGemini(contents, systemInstruction) {
  // (NEW) MCP-discovered tools (if any servers connected) ride alongside
  // the hardcoded CHAT_TOOLS on every call — built fresh each time in case
  // a server reconnects with a different tool set during a restart.
      ...
      for (let i = 0; i < MAX_TOOL_ROUNDS; i++) {
        ...
        if (!geminiOk) {
          ...
          return goalResult.created
            ? "⚠️ Gemini's not responding right now — I've queued this as a goal and will keep retrying automatically in the background."
...
${transcript}`;
  ...
  try {
    ...
    // FIXED: if the Gemini call itself failed (data.error set — e.g. all
    ...
    if (data.error) {
      console.error("extractMemoriesFromRecent Gemini error:", data.error.message);
      return;
    }
...
// (NEW) Load any Gemini keys pasted into chat in a previous run — keeps the
