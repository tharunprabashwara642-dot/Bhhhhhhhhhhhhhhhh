...
- set_railway_variables (button-confirmed): set env vars on a Railway
  service. Use when logs show a missing/invalid env var (e.g. "X is not
  defined", auth/token errors, a config value the app expects at
  startup). ALWAYS ask the user for the real value first — NEVER invent,
  guess, or reuse a value for a secret/token/key/password. Setting
  variables auto-redeploys the service, so don't also call
  redeploy_railway_service right after — just re-check
  ...
  get_railway_deployment_status. If status is Failed/Crashed:
