...
async function fetchNvidiaRotating(url, bodyObj) {
...
// (UPDATED) qwen2.5-coder-32b-instruct reached end-of-life on NVIDIA's
// catalog (2026-05-12) — qwen3-coder-480b-a35b-instruct is the current
// agentic-coding model with tool-calling support. Override via
// NVIDIA_TEXT_MODEL in Railway if NVIDIA retires this one too.
const NVIDIA_TEXT_MODEL = process.env.NVIDIA_TEXT_MODEL || "qwen/qwen3-coder-480b-a35b-instruct";
...
- Respond with ONLY the raw HTML, starting at <!DOCTYPE html> — no markdown code fences, no explanation before or after.`;
    ...
    const data = await nvidiaChatShimmed([{ role: "user", parts: [{ text: codePrompt }] }], null, null);
...
async function nvidiaChatShimmed(contents, systemInstruction, tools, modelOverride) {
  const messages = [];
  ...
  const data = await fetchNvidiaRotating(`${NVIDIA_API_BASE}/chat/completions`, body);
