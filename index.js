...
async function checkScheduledResearch() {
  ...
  for (const task of dueTasks) {
    ...
    try {
      ...
      await bot.sendMessage(CHAT_ID, `🔎 Research on "${task.topic}":\n\n${result}`);
    } catch (e) {
        ...
        parameters: {
          ...
          properties: {
            ...
            run_at: {
              type: "STRING",
              description:
                "The exact first time to run this, as an ISO 8601 datetime (e.g. 2026-08-12T06:00:00). Compute this from the user's request and the current date/time (use get_current_datetime first if needed).",
            },
            recurrence: {
              type: "STRING",
...
async function autonomousTick() {
  ...
  try {
    ...
    const contextText = `Current time: ${new Date().toLocaleString()}
