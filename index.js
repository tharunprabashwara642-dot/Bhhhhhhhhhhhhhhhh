...
BE PROACTIVE, BUT CONFIRM FIRST: don't wait for the user to explicitly say
"add this to my calendar" or "send an email about this." If something they
mention in normal conversation clearly implies an actionable task — an
appointment, a deadline, something that should be emailed to someone, a
reminder — notice it yourself and ASK if they want you to do it (e.g. "That
sounds like something for the calendar — want me to add it?"). Only call
the actual tool (create_calendar_event, send_gmail, etc.) after they
confirm. Never take a real-world action like sending an email on an
inferred task without that confirmation.
...
- send_gmail: Send email (only after the user has confirmed, in this
...
- create_calendar_event: Add event to calendar (only after confirmation
  if the task was inferred rather than directly requested)
...
- create_drive_folder: Create a new folder in Google Drive (needs
  drive.file/drive scope — will fail with drive.readonly)
- create_google_doc: Create a new Google Doc, optionally with initial text
  (needs the documents scope, not documents.readonly)
...
async function executeFunctionCall(fc) {
  try {
    ...
    if (fc.name === "create_drive_folder") return await createDriveFolder(fc.args?.name || "Untitled folder", fc.args?.parent_id);
    if (fc.name === "create_google_doc") return await createGoogleDoc(fc.args?.title || "Untitled document", fc.args?.content || "");
    if (fc.name === "create_google_sheet") return await createGoogleSheet(fc.args?.title || "Untitled spreadsheet");
    if (fc.name === "update_sheet_data") return await updateSheetData(fc.args?.spreadsheet_id, fc.args?.range, fc.args?.values || []);
