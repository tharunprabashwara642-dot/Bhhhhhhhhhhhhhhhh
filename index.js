...
async function fetchNvidiaRotating(url, bodyObj, timeoutMs = 90000) {
...
function toolOutcomeTag(result) {
  ...
  const failed = !!result.error || Object.values(result).some((v) => v === false);
  if (!failed) return "✅";
...
async function cancelAllGoals() {
  ...
  for (const g of data || []) trackedActiveGoals.delete(g.id);
...
assume. Always include the +05:30 offset in run_at.`;
    ...
    try {
      for (let i = 0; i < MAX_TOOL_ROUNDS; i++) {
        let data;
        let brainOk = false;
        // Retry transient NVIDIA failures a couple of times before giving
        // up — a single network hiccup used to dead-end the whole request.
        for (let attempt = 0; attempt < 3 && !brainOk; attempt++) {
          try {
            data = await callBrain(contents, systemInstruction);
            brainOk = true;
          } catch (e) {
            console.error(`callBrain failed (attempt ${attempt + 1}/3):`, e.message);
