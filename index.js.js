...
//
// ============================================================
// NVIDIA MIGRATION (this update) — replaces Gemini as the text/tool-
// calling/coding/vision brain. Add these in Railway → Variables:
//   NVIDIA_API_KEY                 (required — get one at build.nvidia.com.
//                                   Comma-separate multiple keys, or use
//                                   NVIDIA_API_KEYS, for round-robin rotation
//                                   the same way GEMINI_API_KEYS used to.)
//   NVIDIA_TEXT_MODEL               (default: qwen/qwen2.5-coder-32b-instruct)
//   NVIDIA_VISION_MODEL             (default: meta/llama-3.2-90b-vision-instruct
//                                    — used only for photo/image summaries)
//   NVIDIA_EMBEDDING_MODEL          (default: nvidia/nv-embedqa-e5-v5)
//   NVIDIA_ROUGH_DAILY_LIMIT_PER_KEY (rough requests/day per key, for the
//                                    usage-warning estimate — default 1000)
//
// GEMINI_API_KEY is now OPTIONAL — Gemini is used ONLY by the real-time
...
// memory, photo summaries) runs on NVIDIA. Two known gaps from this
// migration, flagged in the code rather than silently broken:
//   - Voice notes (Telegram voice messages) aren't transcribed anymore —
//     NVIDIA's chat API has no audio-input equivalent to Gemini's. Needs a
//     separate speech-to-text integration to bring back.
//   - Non-image documents (PDF, docx, etc.) can't be summarized — NVIDIA's
//     vision model only accepts actual images, unlike Gemini which could
//     read arbitrary file bytes directly. They still get saved to Drive.
//
// Run once in the Supabase SQL editor (rename the old column if you already
// have it): 
//   alter table api_usage add column if not exists nvidia_calls integer not null default 0;
//
// NEW in this update — run once in the Supabase SQL editor:
//
//   -- usage tracking (NVIDIA calls / Vercel deploys per day)
...
// https://build.nvidia.com (click "Get API Key"). Add it in Railway's
// Variables tab as NVIDIA_API_KEY (or NVIDIA_API_KEYS, comma-separated,
// for multiple keys that round-robin the same way the old Gemini pool did).
// ============================================================
let NVIDIA_API_KEYS = (process.env.NVIDIA_API_KEYS || process.env.NVIDIA_API_KEY || "")
  .split(",")
  .map((k) => k.trim())
  .filter(Boolean);

if (NVIDIA_API_KEYS.length === 0) {
  console.error('❌ No NVIDIA API keys configured! Set NVIDIA_API_KEY in Railway → Variables.');
}

console.log(`✅ Loaded ${NVIDIA_API_KEYS.length} NVIDIA API key(s)`);

let nvidiaKeyCursor = 0;
function nextNvidiaKey() {
  if (NVIDIA_API_KEYS.length === 0) return null;
  const k = NVIDIA_API_KEYS[nvidiaKeyCursor % NVIDIA_API_KEYS.length];
  const maskedKey = k.slice(-4);
  console.log(`🔑 Using NVIDIA key index ${nvidiaKeyCursor % NVIDIA_API_KEYS.length} (****${maskedKey})`);
  nvidiaKeyCursor++;
  return k;
}

// ---- chat-provided NVIDIA keys join the rotation pool live (mirrors the
// old Gemini credential-inbox flow) ----
async function loadExtraNvidiaKeysFromDb() {
  try {
    const { data, error } = await supabase
      .from("agent_secrets")
      .select("key_name, value")
      .like("key_name", "NVIDIA_API_KEY_EXTRA_%");
    if (error) {
      if (!/does not exist|relation/i.test(error.message || "")) console.error("loadExtraNvidiaKeysFromDb error:", error.message);
      return;
    }
    let added = 0;
    for (const row of data || []) {
      const v = String(row.value || "").trim();
      if (v && !NVIDIA_API_KEYS.includes(v)) { NVIDIA_API_KEYS.push(v); added++; }
    }
    if (added > 0) console.log(`🔑 Loaded ${added} extra NVIDIA key(s) from agent_secrets — ${NVIDIA_API_KEYS.length} total now active`);
  } catch (e) {
    console.error("loadExtraNvidiaKeysFromDb error:", e.message);
  }
}

async function addNvidiaKeyToPool(rawKey) {
  const key = String(rawKey || "").trim();
  if (!key) return { added: false, reason: "Empty key." };
  if (NVIDIA_API_KEYS.includes(key)) {
    return { added: false, reason: "That key is already in the rotation.", total_keys: NVIDIA_API_KEYS.length };
  }
  NVIDIA_API_KEYS.push(key);
  const slot = NVIDIA_API_KEYS.length - 1;
  const r = await saveSecret(`NVIDIA_API_KEY_EXTRA_${slot}`, key, "NVIDIA key added via Telegram chat credential inbox — joins the rotation pool.");
  if (!r.saved) console.error("addNvidiaKeyToPool: failed to persist key for restart-survival:", r.reason);
  return { added: true, total_keys: NVIDIA_API_KEYS.length, persisted: r.saved };
}
...
const CREDENTIAL_PATTERNS = [
  ...
  // NVIDIA API keys (build.nvidia.com) start with nvapi- — routed into the
  // NVIDIA rotation pool the same way an AIza... key joins the Gemini pool.
  { kind: "nvidia_key", re: /\bnvapi-[0-9A-Za-z_\-]{20,}\b/ },
  { kind: "api_token", re: /\b(?:sk-[A-Za-z0-9_\-]{16,}|ghp_[A-Za-z0-9]{20,}|github_pat_[A-Za-z0-9_]{20,}|AIza[0-9A-Za-z_\-]{20,}|nvapi-[0-9A-Za-z_\-]{20,}|xox[baprs]-[A-Za-z0-9\-]{10,}|[A-Za-z0-9_\-]{32,})\b/ },
];
    ...
    try {
      if (item.kind === "gemini_key") {
        ...
        results.push(r.added
          ...
          : `ℹ️ ${r.reason}`);
      } else if (item.kind === "nvidia_key") {
        const r = await addNvidiaKeyToPool(item.value);
        results.push(r.added
          ? `🔑 NVIDIA key added to the rotation — now cycling across ${r.total_keys} key(s)${r.persisted ? "" : " (⚠️ couldn't persist for restart — re-paste after redeploy)"}.`
          : `ℹ️ ${r.reason}`);
...
// ---- NVIDIA equivalent of fetchGeminiRotating — used by everything now:
// chat, tool-calling, coding/debugging, digest, research, memory, and
// image (photo/document) understanding. NVIDIA's API is OpenAI-compatible,
// so the key goes in an Authorization header instead of the URL, and the
// URL itself is fixed (not per-key) — pass the endpoint in explicitly since
// chat completions and embeddings use different paths.
// (FIX) Default was 90s per key attempt. Combined with handleChatMessage's
// own 3x outer retry AND up to MAX_TOOL_ROUNDS=10 rounds per turn, a genuinely
// slow/stuck NVIDIA endpoint could stack up to several minutes of silent
// waiting before the user saw anything ("processing takes forever" /
// "sometimes no reply at all"). 60s is still generous for a single
// completion and cuts the worst-case stacked wait dramatically. Callers
// that legitimately need longer (e.g. full single-file website generation)
// already pass their own explicit timeoutMs and are unaffected.
async function fetchNvidiaRotating(url, bodyObj, timeoutMs = 60000) {
  const attempts = Math.max(NVIDIA_API_KEYS.length, 1);
  let lastErr;
  for (let i = 0; i < attempts; i++) {
    ...
    try {
      ...
      const data = await res.json();
      if (res.status === 429) {
        console.error(`⚠️ NVIDIA key index ${nvidiaKeyCursor - 1} (****${key.slice(-4)}) rate-limited`);
        lastErr = new Error("Rate limited");
        continue;
      }
      if (!res.ok && res.status >= 500) {
        console.error(`⚠️ NVIDIA key index ${nvidiaKeyCursor - 1} (****${key.slice(-4)}) server error ${res.status}`);
        lastErr = new Error(data.error?.message || `HTTP ${res.status}`);
        continue;
      }
      if (!res.ok) {
        // Non-retryable client error (bad request etc) — surface it
        // directly instead of burning through the whole key pool.
        return { error: data.error || { message: data.detail || `HTTP ${res.status}` } };
      }
      incrementUsage("nvidia_calls"); // fire-and-forget, doesn't block the response
      return data;
    } catch (e) {
      if (e.name === "AbortError") {
        console.error(`⚠️ NVIDIA key index ${nvidiaKeyCursor - 1} (****${key.slice(-4)}) timed out`);
        lastErr = new Error("NVIDIA request timed out");
      } else {
        lastErr = e;
      }
    }
  }
  throw lastErr || new Error("All NVIDIA API keys failed");
}

async function getUsageStats() {
  const today = nowInTimezone().iso.slice(0, 10);
  const { data } = await supabase.from("api_usage").select("*").eq("date", today).maybeSingle();
  const nvidiaCallsToday = data?.nvidia_calls || 0;
  ...
  const roughLimitPerKey = parseInt(process.env.NVIDIA_ROUGH_DAILY_LIMIT_PER_KEY || "1000", 10);
  const roughTotalLimit = NVIDIA_API_KEYS.length * roughLimitPerKey;
  return {
    date: today,
    nvidia_calls_today: nvidiaCallsToday,
    nvidia_keys_active: NVIDIA_API_KEYS.length,
    rough_daily_nvidia_limit_estimate: roughTotalLimit,
    percent_of_rough_limit_used: roughTotalLimit > 0 ? Math.round((nvidiaCallsToday / roughTotalLimit) * 100) : null,
    vercel_deploys_today: vercelDeploysToday,
    note: "These are counts this bot tracks itself, not live numbers pulled from NVIDIA/Vercel's own dashboards — treat the limit as a rough estimate.",
  };
}
...
setInterval(async () => {
  try {
    ...
    if (!stats.rough_daily_nvidia_limit_estimate) return;
    const ratio = stats.nvidia_calls_today / stats.rough_daily_nvidia_limit_estimate;
    ...
    if (ratio >= threshold && usageWarnedDate !== stats.date) {
      ...
      await bot.sendMessage(
        CHAT_ID,
        `⚠️ Boss, අද NVIDIA API calls ${stats.nvidia_calls_today}ක් — rough estimate limit එකෙන් ~${stats.percent_of_rough_limit_used}%ක් පාවිච්චි වෙලා (keys ${stats.nvidia_keys_active}ක් active). අවශ්‍ය නම් NVIDIA_API_KEYS එකට තවත් key එකක් දාන්න.`
      );
    }
  } catch (e) {
...
// Coding-strong instruct model — handles chat, tool-calling, coding/
// debugging, and website generation.
// (UPDATED AGAIN) Third-party Qwen models on NVIDIA's catalog have been
// getting deprecated in quick succession (qwen2.5-coder-32b-instruct EOL
// 2026-05-12, qwen3-coder-480b-a35b-instruct EOL 2026-06-11). Switching to
// NVIDIA's OWN model instead of a third-party-licensed one — NVIDIA has no
// reason to pull its own flagship agentic model the way it pulls licensed
// third-party ones. nemotron-3-super-120b-a12b is purpose-built for
// agentic coding/tool-calling with a 1M token context. Still overridable
// via NVIDIA_TEXT_MODEL in Railway if this one is retired too.
const NVIDIA_TEXT_MODEL = process.env.NVIDIA_TEXT_MODEL || "nvidia/nemotron-3-super-120b-a12b";
// Vision-language model for photo/document image understanding (the
// text model above has no image input support).
...
- get_usage_stats: check today's self-tracked NVIDIA call count and Vercel
...
const CHAT_TOOLS = [
  {
    functionDeclarations: [
      ...
      },
      {
        ...
        description: "Check today's NVIDIA API call count and Vercel deploy count, so you know how close to free-tier limits things are. These are self-tracked counts, not a live pull from NVIDIA/Vercel's dashboards.",
        parameters: { type: "OBJECT", properties: {} },
      },
      {
...
async function getEmbedding(text) {
  try {
    // NVIDIA's NIM embedding models are OpenAI-compatible but require an
    // input_type ("query" vs "passage") — "passage" is the safe default
    // for storing/searching arbitrary memory text.
    const data = await fetchNvidiaRotating(`${NVIDIA_API_BASE}/embeddings`, {
      model: NVIDIA_EMBEDDING_MODEL,
      input: [text],
      input_type: "passage",
    });
    if (data.error) { console.error("getEmbedding error:", data.error.message || JSON.stringify(data.error)); return null; }
...
// responses read as data.candidates[0].content.parts. Rather than rewrite
// every call site's state handling, these two converters translate that
// same shape to/from NVIDIA's OpenAI-compatible chat completions format
// at the boundary — so all the tool-loop logic below is untouched.
// ============================================================
function geminiContentsToOpenAiMessages(contents) {
  const messages = [];
  let pendingToolCalls = []; // [{id, name}] from the most recent assistant turn
  for (const c of contents || []) {
    ...
    if (c.role === "model") {
      ...
      if (fcParts.length) {
        const toolCalls = fcParts.map((p, idx) => ({
          id: `call_${messages.length}_${idx}`,
          type: "function",
          function: { name: p.functionCall.name, arguments: JSON.stringify(p.functionCall.args || {}) },
        }));
        messages.push({ role: "assistant", content: text || null, tool_calls: toolCalls });
        pendingToolCalls = toolCalls.map((tc) => ({ id: tc.id, name: tc.function.name }));
      } else {
      ...
      if (frParts.length) {
        frParts.forEach((p, idx) => {
          ...
          messages.push({ role: "tool", tool_call_id: match.id || `call_unknown_${idx}`, content: JSON.stringify(p.functionResponse.response) });
        });
        pendingToolCalls = [];
      } else {
        const contentParts = [];
...
function openAiMessageToGeminiParts(message) {
  const parts = [];
  if (message && message.content) parts.push({ text: message.content });
...
// contents/systemInstruction/tools all in the original Gemini shape;
// returns a Gemini-shaped { candidates: [...] } object (or { error }) so
// every existing caller keeps working unmodified. Pass tools=null for a
// plain single-shot generation with no function-calling.
async function nvidiaChatShimmed(contents, systemInstruction, tools, modelOverride, timeoutMs) {
  const messages = [];
  if (systemInstruction) messages.push({ role: "system", content: systemInstruction });
  messages.push(...geminiContentsToOpenAiMessages(contents));

  const fnDecls = (tools && tools[0] && tools[0].functionDeclarations) || [];
  const body = { model: modelOverride || NVIDIA_TEXT_MODEL, messages };
  if (fnDecls.length) {
    body.tools = fnDecls.map((fd) => ({
      type: "function",
      function: { name: fd.name, description: fd.description, parameters: fd.parameters || { type: "object", properties: {} } },
    }));
    body.tool_choice = "auto";
  }
  // Nemotron-family models default to a "thinking" mode that emits a long
  // reasoning trace ahead of the actual reply — fine for hard problems, but
  // adds latency and (if not filtered by the endpoint into a separate
  // field) can leak raw reasoning into the chat as if it were the answer.
  // Off by default for a snappy Telegram bot; set NVIDIA_ENABLE_THINKING=true
  // in Railway to turn it back on for harder tasks.
  ...
  const data = await fetchNvidiaRotating(`${NVIDIA_API_BASE}/chat/completions`, body, timeoutMs);
  if (data.error) return { error: data.error };
  const message = data.choices?.[0]?.message || {};
  if (!data.choices?.[0]) return { error: { message: "NVIDIA returned no choices/message." } };
  return { candidates: [{ content: { role: "model", parts: openAiMessageToGeminiParts(message) } }] };
}
...
async function handleChatMessage(userText) {
  if (NVIDIA_API_KEYS.length === 0) {
    return "⚠️ No NVIDIA API keys configured. Set NVIDIA_API_KEY in Railway → Variables.";
  }
...
async function extractMemoriesFromRecent() {
  if (NVIDIA_API_KEYS.length === 0) return;
...
bot.on("message", async (msg) => {
  ...
  if (msg.document || (msg.photo && msg.photo.length > 0)) {
    try {
      ...
      // NVIDIA vision models (unlike Gemini) only understand actual image
      // bytes — a non-image document (PDF, docx, etc.) sent as image_url
      // won't decode. Route images through the NVIDIA vision model; for
      // other file types, say so plainly instead of returning a fake summary.
      let summary;
      if (mimeType.startsWith("image/")) {
        const summaryData = await nvidiaChatShimmed(
          [{
            ...
            parts: [
              ...
              { text: "Summarize this image concisely (4-6 sentences, no markdown, no headers) — what it is and the key points a busy person needs to know." },
            ],
          }],
          null,
          null,
        ...
        summary = (summaryData.candidates?.[0]?.content?.parts || []).map((p) => p.text || "").join(" ").trim()
          || "Image එක කියවගන්න බැරි උනා.";
      } else {
        summary = `⚠️ මේ file type එකේ (${mimeType}) content එක කෙලින්ම කියවගන්න බැරි උනා — NVIDIA vision model එකට images විතරයි තේරෙන්නෙ, PDF/document files වලට වෙනම text-extraction step එකක් ඕන. Drive එකට save කරන්නම් (below).`;
      }
    ...
    if (lower === "stats") {
      try {
        ...
        const msgText = `📊 *Quick Stats Summary*\n\n` +
          `- Today's NVIDIA API Calls: *${usage.nvidia_calls_today}*\n` +
          `- Today's Vercel Deploys: *${usage.vercel_deploys_today}*\n` +
          `- Active Goals: *${activeCount}*\n` +
          `- Active NVIDIA Keys: *${usage.nvidia_keys_active}*`;
        await sendLongMessage(CHAT_ID, msgText);
        await logBotMessage("agent", msgText);
      } catch (err) {
...
// STARTUP
// ============================================================
console.log(`🚀 Night Agent started with ${NVIDIA_API_KEYS.length} NVIDIA key(s) (+ ${API_KEYS.length} Gemini key(s), voice-only)`);
...
console.log(`✅ Model: ${NVIDIA_TEXT_MODEL} (vision: ${NVIDIA_VISION_MODEL})`);
